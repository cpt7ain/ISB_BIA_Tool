...
namespace ISB_BIA_IMPORT1.Services
{
    class ExportService : IExportService
    {
        ...
        public ExportService(IDialogService myDia, IDataService_Setting mySett, 
            IDataService_Application myApp, IDataService_Process myProc,
            IDataService_Segment myIS, ISharedResourceService mySharedResourceService)
        {
            this._myDia = myDia;
            this._mySett = mySett;
            this._myIS = myIS;
            ...
            if (!_myShared.Conf_ConstructionMode) Setting = _mySett.Get_List_Settings();
            if (!_myShared.Conf_ConstructionMode) ISList = _myIS.Get_List_Segments_Enabled();
        }
        ...
        public bool Export_Processes_Active()
        {
            ...
            ps = _myProc.Get_List_Processes_Active();
            if (ps != null)
                ...
                if (result == true)
                {
                    try
                    {
                        ...
                        for (int i = 0; i < attributeList.Count; i++)
                        {
                            ...
                            cell = row.CreateCell(11);
                            cell.SetCellValue(attributeList[i].Benutzer);
                        }
                        #endregion
